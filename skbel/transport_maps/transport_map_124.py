...
class transport_map():
    ...
    def __init__(self, monotone, nonmonotone, X, polynomial_type='hermite function', 
        monotonicity = 'integrated rectifier', standardize_samples = True, 
        standardization = 'standard', workers = 1, ST_scale_factor = 1.0, 
        ST_scale_mode = 'dynamic', coeffs_init = 0.1, verbose = True, linearization = None, 
        linearization_specified_as_quantiles = True, linearization_increment = 1E-6, 
        regularization = None, regularization_lambda = 0.1, 
        quadrature_input = {}, rectifier_type = 'exponential',
        delta = 0.):
        ...
        Variables:
            ...
            delta - [default = 0.]
                [float] : small increment added to the rectifier to prevent
            ...
            # -----------------------------------------------------------------
                ...
                if mode == 'standard':
                    ...
                    if int(i) == k:
                        ...
                        string      = "(__x__[...,"+i+"] - __mu__)/(np.sqrt(2*np.pi)*__scale__**3)*np.exp(-(__x__[...,"+i+"] - __mu__)**2/(2*__scale__**2))"
        ...
        for k in range(K):
            ...
            for i,entry in enumerate(self.monotone[k]):
                ...
                if "ST" in list(modifier_log.keys()):
                    ...
                    term    = term.replace(
                        "__mu__",
                        "self.ST_centers_m["+str(k)+"]["+str(idx)+"]["+str(ST_counter[idx])+"]")
                    
                    # Replace __scale__ with the correct ST location variable
                    term    = term.replace(
                        "__scale__",
                        "self.ST_scales_m["+str(k)+"]["+str(idx)+"]["+str(ST_counter[idx])+"]")
            ...
            for entry in self.nonmonotone[k]:
                ...
                if "ST" in list(modifier_log.keys()):
                    ...
                    term    = term.replace(
                        "__mu__",
                        "self.ST_centers_nm["+str(k)+"]["+str(idx)+"]["+str(ST_counter[idx])+"]")
                    
                    # Replace __scale__ with the correct ST location variable
                    term    = term.replace(
                        "__scale__",
                        "self.ST_scales_nm["+str(k)+"]["+str(idx)+"]["+str(ST_counter[idx])+"]")
    ...
    def calculate_special_term_locations(self):
        ...
        for k in range(self.D):
            ...
            for idx,RBF_counter in enumerate([self.RBF_counter_m,self.RBF_counter_nm]):
                    ...
                    for d in range(self.X.shape[-1]):
                        ...
                        # ---------------------------------------------------------
                            ...
                            if idx == 0:    # monotone
                                ...
                                self.ST_centers_m[-1]   .append([np.quantile(
                                    self.X[:,d],
                                    q   = 0.5)])
                                
                                if self.ST_scale_mode == 'dynamic':
                                    self.ST_scales_m[-1]    .append([self.ST_scale_factor/2])
                                elif self.ST_scale_mode == 'static':
                                    self.ST_scales_m[-1]    .append([self.ST_scale_factor])
                                
                            elif idx == 1:  # nonmonotone
                            
                                self.ST_centers_nm[-1]  .append([np.quantile(
                                    self.X[:,d],
                                    q   = 0.5)])
                        ...
                        # ---------------------------------------------------------
                            ...
                            quantiles   = np.arange(1,RBF_counter[k,d]+1,1)/(RBF_counter[k,d]+1)
                                ...
                                if self.ST_scale_mode == 'dynamic':
                                    ...
                                    for i in range(RBF_counter[k,d]):
                                        ...
                                        if i == 0:
                                            ...
                                            scales[i] = \
                                                (self.ST_centers_m[-1][d][1] - self.ST_centers_m[-1][d][0])/2*self.ST_scale_factor
                                            
                                        # Right edge-case: base is half distance to previous basis
                                        elif i == RBF_counter[k,d]-1:
                                            
                                            scales[i] = \
                                                (self.ST_centers_m[-1][d][i] - self.ST_centers_m[-1][d][i-1])/2*self.ST_scale_factor
                                        
                                        # Otherwise: base is average distance to neighbours
                                        else:
                                            
                                            scales[i] = \
                                                (self.ST_centers_m[-1][d][i+1] - self.ST_centers_m[-1][d][i-1])/2*self.ST_scale_factor
                                    ...
                                    self.ST_scales_m[-1]    .append(copy.copy(scales) + self.ST_scale_factor)
                                ...
                                if self.ST_scale_mode == 'dynamic':
                                    ...
                                    for i in range(RBF_counter[k,d]):
                                        ...
                                        if i == 0:
                                            ...
                                            scales[i] = \
                                                (self.ST_centers_nm[-1][d][1] - self.ST_centers_nm[-1][d][0])/2*self.ST_scale_factor
                                            
                                        # Right edge-case: base is half distance to previous basis
                                        elif i == RBF_counter[k,d]-1:
                                            
                                            scales[i] = \
                                                (self.ST_centers_nm[-1][d][i] - self.ST_centers_nm[-1][d][i-1])/2*self.ST_scale_factor
                                        
                                        # Otherwise: base is average distance to neighbours
                                        else:
                                            
                                            scales[i] = \
                                                (self.ST_centers_nm[-1][d][i+1] - self.ST_centers_nm[-1][d][i-1])/2*self.ST_scale_factor
                                    ...
                                    self.ST_scales_nm[-1]   .append(copy.copy(scales) + self.ST_scale_factor)
    ...
    def worker_task_monotone(self,k,task_supervisor):
        ...
        Psi_mon     = copy.copy(self.fun_mon[k](
            copy.copy(self.X),
            self))
        
        Psi_nonmon  = copy.copy(self.fun_nonmon[k](
            copy.copy(self.X),
            self))
        ...
        if self.regularization is None:
            ...
            def fun_mon_objective(coeffs_mon,A,k):
                ...
                objective   = np.linalg.multi_dot((
                    coeffs_mon[:,np.newaxis].T,
                    A,
                    coeffs_mon[:,np.newaxis]))[0,0]/2
            ...
            A   = np.linalg.multi_dot((
                np.linalg.inv(
                    np.dot(
                        Psi_nonmon.T,
                        Psi_nonmon) + self.regularization_lambda*np.identity(Psi_nonmon.shape[-1])),
                Psi_nonmon.T,
                Psi_mon))
    ...
    def vectorized_root_search_bisection(self, X, Yk, k, max_iterations = 100, 
        ...
        while len(shiftindices) > 0:
            ...
            bsct_out[shiftindices[failure_pos],0]   = copy.copy(self.s(
                ...
                k       = k) - Yk[shiftindices[failure_pos]])
            ...
            bsct_out[shiftindices[failure_neg],1]   = copy.copy(self.s(
                ...
                k       = k) - Yk[shiftindices[failure_neg]])
